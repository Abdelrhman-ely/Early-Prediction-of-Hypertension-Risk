...
# ---------------------------
# Load Pipeline (Dynamic Path)
# ---------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
pipeline_path = os.path.join(BASE_DIR, "xgb_pipeline.pkl")

try:
    with open(pipeline_path, "rb") as f:
        pipeline = pickle.load(f)
except FileNotFoundError:
    st.error("❌ File xgb_pipeline.pkl not found. Please upload it in the same folder as this app.")
    st.stop()
